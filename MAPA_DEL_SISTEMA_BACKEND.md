...
>
> Generado: 2026-08-10. Actualizado: 2026-08-11 (prevención de duplicados en turnos_rtms, Regla 1 de turno abierto, cancelación con motivo obligatorio, campos sensibles editables en dateos, comisión manual marca dateo EXITOSO). Si el código cambia, este documento puede quedar desactualizado — verificar contra el código real antes de confiar ciegamente en detalles finos (nombres exactos de columnas, roles).
...
- `app/controllers/captacion_dateos_controller.ts`
  ...
  - `PUT /captacion-dateos/:id`, `DELETE /captacion-dateos/:id` (solo SUPER_ADMIN/GERENCIA). **🆕** `PUT` ahora valida y persiste `agente_id`/`convenio_id`/`servicio_id` (campos "sensibles" — antes el `request.only()` de `update()` no los leía en absoluto, aunque la ruta ya estaba restringida a estos roles). `GET /captacion-dateos/:id` (`show`) ahora también precarga `servicio` y expone `agente_id`/`convenio_id`/`servicio_id` como alias snake_case explícitos en la respuesta (antes solo viajaban como `agenteId`/`convenioId`/`servicioId` camelCase, sin alias, así que el frontend no podía prellenar un formulario de edición con ellos).
