...
- `app/controllers/captacion_dateos_controller.ts`
  ...
  - `POST /captacion-dateos` (no CONTABILIDAD) — crea un dateo. **🆕 Bug fix (dateos huérfanos):** antes de crear, llama `cerrarDateosViejosPorPlacaTelefono()` para cerrar como `REEMPLAZADO` cualquier dateo viejo en `RE_DATEAR` de esa misma placa/teléfono (sin transacción propia — `create()` es una sola sentencia atómica).
  ...
  - `PUT /captacion-dateos/:id`, `DELETE /captacion-dateos/:id` (solo SUPER_ADMIN/GERENCIA). **🆕** `PUT` ahora valida y persiste `agente_id`/`convenio_id`/`servicio_id` (campos "sensibles" — antes el `request.only()` de `update()` no los leía en absoluto, aunque la ruta ya estaba restringida a estos roles). `GET /captacion-dateos/:id` (`show`) ahora también precarga `servicio` y expone `agente_id`/`convenio_id`/`servicio_id` como alias snake_case explícitos en la respuesta (antes solo viajaban como `agenteId`/`convenioId`/`servicioId` camelCase, sin alias, así que el frontend no podía prellenar un formulario de edición con ellos). **🆕** `show()` también devuelve `documentosInformativos` — array de tickets de `Facturación` vinculados a este dateo (por `dateo_id`, con fallback por `turno_id` al `consumidoTurnoId` del dateo) cuyo descuento aplicado tiene un código que empieza por `INFORMATIVO` (ej. `INFORMATIVO_POLICIA`) y al menos uno de los 3 documentos de verificación (`doc_carnet_path`/`doc_tarjeta_propiedad_path`/`doc_cedula_path`) subido; cada entrada expone `ticketId`, `descuentoCodigo` y las 3 rutas de documento. Antes esta evidencia solo era visible desde el ticket de Facturación, no desde la ficha del dateo. **🆕** `PUT` bloquea las transiciones manuales de/hacia `RE_DATEAR`: `resultado='RE_DATEAR'` en el body → 422 ("solo lo asigna el job de verificación de vencidos"); si el dateo ya está en `RE_DATEAR`, cualquier intento de cambiar `resultado` → 422 ("usa `POST /:id/redatear`, requiere evidencia"). El resto de transiciones de `resultado` sigue sin restricción.
