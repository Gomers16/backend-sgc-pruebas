...
- **`ComisionesDetail.vue`** (`src/views/comercial/comisiones/`) — tiene ruta registrada (`/comercial/comisiones/:id`, nombre `ComercialComisionDetalle`) pero **nada en la UI navega ahí**: búsqueda exhaustiva en todo `src/` sin ningún `router.push`/`<router-link>` a ese nombre de ruta. El modal real de "Detalle comisión #XXX" que abre el ícono del ojo en `ComisionesList.vue` es un `detailDialog` **inline** dentro del mismo archivo (líneas ~1743-2045), con su propia lógica de "¿por qué se paga esto?" (`buildJustificacion()`, 100% heurística en el frontend, no usa el `reglaAplicada` que ya calcula `calcularComision()` en el backend y que hoy solo se `console.log()`ea — deuda pendiente de evaluar) — no este componente. Bonus: el botón "volver" de `ComisionesDetail.vue` apunta a `router.push({ name: 'comercial.comisiones.list' })`, un nombre de ruta que **no existe** (la ruta real se llama `ComercialComisiones`) — fallaría silenciosamente si alguien llegara ahí por URL directa.
